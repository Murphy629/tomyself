<template>
  ...
  >
    ...
    <!-- Middle column: nav scrolls; user card pinned at bottom -->
    <div class="scroll">
      ...
      <div class="theme-selector">
          <ThemeSwitch v-model="theme" @change="onThemeChange" />
      </div>

    </div>

    <!-- Resize handle -->
    <div class="resize-handle" title="Drag to resize" @mousedown="onMouseDown" />
  </aside>
</template>
...
const theme = ref('adaptive') // 'light' | 'adaptive' | 'dark'
...
// Active check: exact for '/', prefix for others (so /about/x is active)
const isActive = (item) => {
...
.sidebar {
  position: sticky;  /* stays in view when the page scrolls */
  top: 0;
  height: 100vh;     /* exactly window height */
  background: var(--bg);
  color: var(--text);
  border-right: 1px dotted rgb(183, 183, 183);
  display: flex;
  flex-direction: column;
  overflow: hidden;  /* no sidebar scroll; inner areas control it */
}
...
.scroll {
  ...
  flex-direction: column;
  height: 100%;
  padding: 8px;       /* no extra bottom pad needed now */
  overflow: hidden;   /* contain internal scrolling */
}

.nav-scroll {
  flex: 1 1 auto;     /* takes remaining height */
  min-height: 0;      /* critical for scroll containers in flex */
  overflow-y: auto;   /* only the nav list scrolls if it gets long */
}
