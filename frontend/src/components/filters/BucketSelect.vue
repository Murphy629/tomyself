<template>
  <div class="select">
    ...
    <ul v-if="open" class="menu" role="listbox">
      ...
      >
        <span>{{ opt }}</span>
        <span v-if="opt === modelValue" class="tick">✓</span>
      </li>
...
const props = defineProps({
  ...
  placeholder: { type: String, default: 'Bucket' },
...
// 点击外部关闭
const onDoc = (e) => {
  if (!e.target.closest('.select')) open.value = false
}
...
.menu {
  position: absolute; z-index: 10; margin-top: 6px; width: 100%;
